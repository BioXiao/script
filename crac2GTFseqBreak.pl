...
while(<CRACFILE>)
{
	...
	foreach my $breakPos (split(/\//,$tmp))
	{
	    ...
	    if(defined $expId)
	    {
		print OUTFILE "$chr\tcrac\tfusion\t$start\t$end\t.\t$str\t.\tgene_id \"$ids[$geneNum]\"; transcript_id \"".$ids[$geneNum]."_$ids[0]/$ids[1]_".$expId."_$break\"; fusion_id \"$ids[0]/$ids[1]\"; experiment_id \"$expId\"; gene_pos \"$genePos\"; break_value \"$break\";\n";
	    }
	    else
	    {
		print OUTFILE "$chr\tcrac\tfusion\t$start\t$end\t.\t$str\t.\tgene_id \"$ids[$geneNum]\"; transcript_id \"".$ids[$geneNum]."_$ids[0]/$ids[1]_$break\"; fusion_id \"$ids[0]/$ids[1]\"; gene_pos \"$genePos\"; break_value \"$break\";\n";
	    }
