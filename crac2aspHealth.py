#!/usr/bin/env python
# -*- coding: utf-8 -*-
# Usage : crac2aspHealth.py healthySample sampleID paralogousFile outFile
...
# Argument test
if len( sys.argv ) != 5:
    print "Error: wrong argument number. Usage: crac2aspHealth.py healthySample sampleID paralogousFile outFile"
    sys.exit()
        ...
        if (name1=='' or name2=='') or \
           ...
           ((name1!='' and not re.search(name1,name2)) and (name2!='' and not re.search(name2,name1))):
            ...
            for breakPoint in tmp[4].split("),("):
                ...
                #                       ('', '', '', '', ''))
                out.write( 'fusionHealth("crac", "%s", ("%s","%s"), ("%s","%s"), ("%s",%s,"%s"), ("%s",%s,"%s"), (%s,%s), ("","","","","")).\n'
                           %(sampleID,
                             ids[0], ids[1],
