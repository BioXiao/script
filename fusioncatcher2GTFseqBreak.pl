...
while(<FCFILE>)
{
	...
	if(defined $expId)
	{
	    print OUTFILE "$chr\tfusioncatcher\texon\t$start\t$end\t.\t$str\t.\tgene_id \"$ids[$geneNum]\"; transcript_id \"$ids[0]/$ids[1]_".$expId."_$line[8]/$line[9]\"; fusion_id \"$ids[0]/$ids[1]\"; experiment_id \"$expId\"; gene_pos \"$genePos\"; break_value \"$line[8]/$line[9]\";\n";
	}
	else
	{
