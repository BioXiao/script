#!/usr/bin/env python
# -*- coding: utf-8 -*-
# Usage : fusioncatcher2asp.py tumorSample sampleID paralogousFile outFile
...
# Argument test
if len( sys.argv ) != 5:
    print "Error: wrong argument number. Usage: fusioncatcher2asp.py tumorSample sampleID paralogousFile outFile"
    sys.exit()
        ...
        if ids not in para:
            ...
            if (name1=='' or name2=='') or \
               ...
               ((name1!='' and not re.search(name1,name2)) and (name2!='' and not re.search(name2,name1))):
                ...
                out.write( 'fusion("fusioncatcher", "%s", ("%s","%s"), ("%s","%s"), ("%s",%s,"%s"), ("%s",%s,"%s"), (%s,%s), ("%s","%s","%s","%s","%s")).\n'
                           %(sampleID,
                             ids[0], ids[1],
